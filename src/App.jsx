...
export default function App() {
  ...
  const countdown = useMemo(()=>{
    ...
    // Always parse from ET (reference TZ for all match data), then apply EDT offset (UTC-4, summer)
    ...
    if(ampm==="AM"&&h===12) h=0;
    // Tournament runs Jun–Jul 2026 → EDT (UTC-4)
    const utcMs = Date.UTC(
      parseInt(nextMatch.date.slice(0,4)),
      parseInt(nextMatch.date.slice(5,7))-1,
      parseInt(nextMatch.date.slice(8,10)),
      h+4, parseInt(mS||"0"), 0
    );
